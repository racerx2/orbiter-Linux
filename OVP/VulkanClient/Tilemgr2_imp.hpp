...
void TileManager2Base::ProcessNode (QuadTreeNode<TileType> *node)
{
	...
	MATRIX4toFMATRIX4(tile->dmWorld, tile->mWorld);
...
template<class TileType>
void TileManager2Base::RenderNodeLabels(QuadTreeNode<TileType> *node, VulkanPad *skp, oapi::Font **labelfont, int *fontidx)
{
	TileType *tile = node->Entry();
