...
class VulkanParticleStream : public oapi::ParticleStream, public VulkanEffect
{
...
public:
	 ...
	 */
	VulkanParticleStream (oapi::GraphicsClient *_gc, PARTICLESTREAMSPEC *pss = 0);

	/**
	 * \brief Destroys the particle stream object
	 */
	virtual ~VulkanParticleStream();
	...
	void   Render(VulkanDevice *dev);
	//void Render(VulkanDevice *dev, VulkanTexture *&prevtex);

	virtual void RenderGroundShadow (VulkanDevice *dev, VulkanTexture *&prevtex) {}
...
protected:
	...
	virtual void SetMaterial (COLOUR4 &col) { col.r = col.g = col.b = 1; }
	void RenderDiffuse (VulkanDevice *dev);
	void RenderEmissive (VulkanDevice *dev);
...
protected:
	oapi::VulkanClient *pGC;					// pointer to graphics client
...
class ExhaustStream: public VulkanParticleStream {
public:
	...
	ExhaustStream (oapi::GraphicsClient *_gc, OBJHANDLE hV,
		...
		PARTICLESTREAMSPEC *pss = 0);
	void RenderGroundShadow (VulkanDevice *dev, VulkanTexture *&prevtex);
	void Update ();
...
class ReentryStream: public VulkanParticleStream {
public:
