...
PlanetShader::PlanetShader(VulkanDevice *pDev, const char* file, const char* vs, const char* ps, const char* name, const char* options)
	: ShaderClass(pDev, file, vs, ps, name, options)
{
	...
	if (std::string(options).find("_DEVTOOLS") != std::string::npos) bDevtools = true;
	if (std::string(options).find("_NO_ATMOSPHERE") != std::string::npos) bAtmosphere = false;
	if (std::string(options).find("_WATER") != std::string::npos) bWater = true;
	if (std::string(options).find("_RIPPLES") != std::string::npos) bRipples = true;
	if (std::string(options).find("_CLOUDSHD") != std::string::npos) bCloudShd = true;
	if (std::string(options).find("_NIGHTLIGHTS") != std::string::npos) bNightlights = true;
...
// ===========================================================================================
//
void vPlanet::GlobalInitAtmosphere(oapi::VulkanClient* gc)
{
	pDev = gc->GetDevice();
	...
	// bClouds and bAtmosphere are read from the config and never used, on
	// both platforms.
	(void)bClouds; (void)bAtmosphere;
...
// ===========================================================================================
//
VulkanSun vPlanet::GetObjectAtmoParams(VECTOR3 vRelPos)
{
	assert(std::string(name) != "Sun");
...
void vPlanet::UpdateScatter()
{
	...
	if (bTrace && nSct < 40) {
		nSct++;
		LogErr("SCATTER %s frame=%u dwSctFrame=%u pass=%u mesh=%p -> %s",
			   name, (unsigned)scn->GetFrameId(), (unsigned)dwSctFrame,
			   (unsigned)scn->GetRenderPass(), (void *)mesh,
			   (scn->GetFrameId() == dwSctFrame) ? "SKIP same frame" :
			   (scn->GetRenderPass() != RENDERPASS_MAINSCENE) ? "SKIP not mainscene" :
			   mesh ? "SKIP mesh body" : "UPDATE");
	}
	...
	if (HasAtmosphere() && surfmgr2)
	{
		...
		const VkFormat tblFormat = VK_FORMAT_R16G16B16A16_SFLOAT;

		if (!pSunColor) pSunColor = pDev->CreateTexture(4*Qc, Qc/2, 1, tblFormat, tblUsage);
		if (!pRaySkyView) pRaySkyView = pDev->CreateTexture(Qc * 2, Qc, 1, tblFormat, tblUsage);
		if (!pMieSkyView) pMieSkyView = pDev->CreateTexture(Qc * 2, Qc, 1, tblFormat, tblUsage);
		if (!pLandViewRay) pLandViewRay = pDev->CreateTexture(Wc * Nc, Wc, 1, tblFormat, tblUsage);
		if (!pLandViewMie) pLandViewMie = pDev->CreateTexture(Wc * Nc, Wc, 1, tblFormat, tblUsage);
		if (!pLandViewAtn) pLandViewAtn = pDev->CreateTexture(Wc * Nc, Wc, 1, tblFormat, tblUsage);
		if (!pAmbientSky) pAmbientSky = pDev->CreateTexture(Qc, Qc, 1, tblFormat, tblUsage);
	}
	...
	if (bTrace && nSct <= 40) {
		...
		LogErr("SCATTER   %s took mVP r0=(% .4f % .4f % .4f % .4f) r3=(% .1f % .1f % .1f % .4f)",
			   name, q.m11, q.m12, q.m13, q.m14, q.m41, q.m42, q.m43, q.m44);
	}
	...
	if (getenv("ORBITER_VK_TRACE_SCT")) {
		...
		if (!bSaid && strcmp(name, "Earth") == 0) {
			bSaid = true;
			LogErr("SCTIN  PlanetRad=%.1f PlanetRad2=%.6e  CamRad=%.1f CamRad2=%.6e  CamAlt=%.1f",
				   cp.PlanetRad, cp.PlanetRad2, cp.CamRad, cp.CamRad2, cp.CamAlt);
			LogErr("SCTIN  AtmoAlt=%.1f AtmoRad=%.1f AtmoRad2=%.6e  Cr2=%.6e  HrzDst=%.1f",
				   cp.AtmoAlt, cp.AtmoRad, cp.AtmoRad2, cp.Cr2, cp.HrzDst);
			LogErr("SCTIN  toCam=(%.4f %.4f %.4f)  toSun=(%.4f %.4f %.4f)",
				   cp.toCam.x, cp.toCam.y, cp.toCam.z, cp.toSun.x, cp.toSun.y, cp.toSun.z);
			LogErr("SCTIN  Up=(%.4f %.4f %.4f)  ZeroAz=(%.4f %.4f %.4f)  SunAz=(%.4f %.4f %.4f)",
				   cp.Up.x, cp.Up.y, cp.Up.z, cp.ZeroAz.x, cp.ZeroAz.y, cp.ZeroAz.z,
				   cp.SunAz.x, cp.SunAz.y, cp.SunAz.z);
			LogErr("SCTIN  CamPos=(%.1f %.1f %.1f)  dCS=%.4f  MinAlt=%.1f MaxAlt=%.1f",
				   cp.CamPos.x, cp.CamPos.y, cp.CamPos.z, cp.dCS, cp.MinAlt, cp.MaxAlt);
			LogErr("SCTIN  cSun=(%.3f %.3f %.3f) rmI=(%.3e %.3e) rmO=(%.3e %.3e) iH=(%.3e %.3e)",
				   cp.cSun.x, cp.cSun.y, cp.cSun.z, cp.rmI.x, cp.rmI.y,
				   cp.rmO.x, cp.rmO.y, cp.iH.x, cp.iH.y);
			LogErr("SCTIN  RayWave=(%.3f %.3f %.3f) MieWave=(%.3f %.3f %.3f)  bCamLit=%d bCamInSpace=%d",
				   cp.RayWave.x, cp.RayWave.y, cp.RayWave.z,
				   cp.MieWave.x, cp.MieWave.y, cp.MieWave.z,
				   int(Flow.bCamLit), int(Flow.bCamInSpace));
			LogErr("SCTIN  sizeof(ConstParams)=%u sizeof(sFlow)=%u",
				   (unsigned)sizeof(ConstParams), (unsigned)sizeof(sFlow));
		}
	}
	...
	if (getenv("ORBITER_VK_TRACE_SCT")) {
		...
		if (!bDone) {
			bDone = true;
			struct { const char *name; VulkanTexture *pT; } tbl[] = {
				{ "SunColor",    pSunColor    },
				{ "RaySkyView",  pRaySkyView  },
				{ "MieSkyView",  pMieSkyView  },
				{ "LandViewRay", pLandViewRay },
				{ "LandViewMie", pLandViewMie },
				{ "LandViewAtn", pLandViewAtn },
				{ "AmbientSky",  pAmbientSky  },
			};
			for (auto &e : tbl) {
				if (!e.pT) { LogErr("SCT %-12s (null)", e.name); continue; }
				const VulkanImageDesc &d = e.pT->Desc();
				const size_t texels = size_t(d.Width) * d.Height;
				const size_t bytes = texels * 8;		// R16G16B16A16_SFLOAT
				std::vector<uint16_t> buf(texels * 4, 0);
				if (!pDev->ReadTexture(e.pT, 0, buf.data(), bytes)) {
					LogErr("SCT %-12s readback FAILED (%ux%u)", e.name, d.Width, d.Height);
					continue;
				}
				// glm-free half -> float, enough for a magnitude check.
				auto h2f = [](uint16_t h) -> float {
					uint32_t s = (h >> 15) & 1, ex = (h >> 10) & 0x1F, m = h & 0x3FF;
					if (ex == 0) return (s ? -1.0f : 1.0f) * float(m) * 5.9604645e-8f;
					if (ex == 31) return m ? 0.0f : (s ? -1e30f : 1e30f);
					uint32_t f = (s << 31) | ((ex - 15 + 127) << 23) | (m << 13);
					float r; memcpy(&r, &f, 4); return r;
				};
				double sum = 0.0; float mx = 0.0f; size_t nz = 0;
				double asum = 0.0; float amx = 0.0f; size_t anz = 0;
				...
				LogErr("SCT %-12s A: mean=%.5f max=%.5f nonzero=%zu/%zu",
					   e.name, asum / double(texels), amx, anz, texels);
				const size_t last = (size_t(d.Height) - 1) * d.Width;
				LogErr("SCT %-12s %4ux%-4u  R: mean=%.5f max=%.5f nonzero=%zu/%zu  row0=%.4f rowN=%.4f",
				...
				for (int k = 0; k < 9; k++) {
					...
					const size_t o = (size_t(row) * d.Width + d.Width / 2) * 4;
					at += snprintf(prof + at, sizeof(prof) - at, " %.3f", h2f(buf[o]));
					if (at >= int(sizeof(prof)) - 8) break;
				}
				LogErr("SCT   %-10s profile v=0..1:%s", e.name, prof);
			}
		}
	}
}
...
VulkanTexture * vPlanet::GetScatterTable(int i)
{
	switch (i)
	{
...
bool vPlanet::LoadAtmoConfig()
{
	...
	if (it != Config->AtmoCfg.end()) {
		...
		Config->AtmoCfg[name] = std::string(name) + ".atm.cfg";
	}
	...
	if (getenv("ORBITER_VK_TRACE_ATMO")) {
		...
		LogErr("ATMO   SPrm suni=%.4f trb=%.4f tgamma=%.4f aux3=%.4f ray=%.4f mie=%.4f rheight=%.4f visalt=%.1f orbalt=%.1f",
			   SPrm.suni, SPrm.trb, SPrm.tgamma, SPrm.aux3, SPrm.ray, SPrm.mie, SPrm.rheight, SPrm.visalt, SPrm.orbalt);
		LogErr("ATMO   OPrm suni=%.4f trb=%.4f tgamma=%.4f aux3=%.4f ray=%.4f mie=%.4f rheight=%.4f",
			   OPrm.suni, OPrm.trb, OPrm.tgamma, OPrm.aux3, OPrm.ray, OPrm.mie, OPrm.rheight);
		LogErr("ATMO   HPrm suni=%.4f trb=%.4f tgamma=%.4f aux3=%.4f ray=%.4f mie=%.4f rheight=%.4f",
			   HPrm.suni, HPrm.trb, HPrm.tgamma, HPrm.aux3, HPrm.ray, HPrm.mie, HPrm.rheight);
	}
...
void vPlanet::SaveAtmoConfig()
{
	...
	oapiWriteItem_string(hFile, (char*)"Planet", this->name);
...
void vPlanet::TestComputations(Sketchpad* pSkp)
{
	...
	(void)hV;	// fetched and never read, on both platforms
	if (hR != hObj) return;
	...
	VulkanDebugLog("Optical Depth=%f   RGB(%f, %f, %f)", rd.a, rd.r, rd.g, rd.b);
	...
	(void)ph;	// computed and never read, on both platforms
	...
	VulkanDebugLog("Sunlight RGB(%f, %f, %f)", cl.x, cl.y, cl.z);
	VulkanDebugLog("InScatter RGB(%f, %f, %f)", is.x, is.y, is.z);
	VulkanDebugLog("RayLength = %f vs %f, sa = %f, sl = %f", rl, rf, sa, sl);
	VulkanDebugLog("AtmoAlt = %f(km)", cp.AtmoAlt/1000.0f);
	...
	if (bSrc) VulkanDebugLog("Camera in Shadow");
	if (bTgt) VulkanDebugLog("Target in Shadow");


	if (status == 1) VulkanDebugLog("SURFACE");
	if (status == 2) VulkanDebugLog("HORIZON");
	if (status == 3) VulkanDebugLog("SKYDOME");

	VulkanDebugLog("Shadow First=%f, Second=%f", sp.se, sp.sx);
	VulkanDebugLog("Atmosp First=%f, Second=%f", sp.ae, sp.ax);
	...
	VulkanDebugLog("s0=%f, e0=%f, s1=%f, e1=%f, mp=%f, lf=%f", s0, e0, s1, e1, mp, lf);

	if (e0 > s0) {
		VulkanDebugLog("Primary Integral");
		V0 = (vR + vRay * s0)._V();
		V1 = V0 + vRay._V() * (e0 - s0);
	...
	if (e1 > s1) {
		VulkanDebugLog("Secondary Integral");
		V0 = (vR + vRay * s1)._V();
		V1 = V0 + vRay._V() * (e1 - s1);
		scn->WorldToScreenSpace(V0, &pt0);
		scn->WorldToScreenSpace(V1, &pt1);
