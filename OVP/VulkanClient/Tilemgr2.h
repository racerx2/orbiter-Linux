...
protected:
	...
	bool	CreateTexture(VulkanDevice *pDev, VulkanTexture *pPre, VulkanTexture **pTex);
	bool	LoadTextureFile(const char *path, VulkanTexture **pPre);
	bool	LoadTextureFromMemory(void *data, DWORD ndata, VulkanTexture **pPre);
...
public:
	explicit TileLoader (const oapi::VulkanClient *gclient);
	~TileLoader ();
...
public:
	 ...
	 */
	static void GlobalInit (class oapi::VulkanClient *gclient);
	...
	static VulkanDevice *Dev() { return pDev; }
	...
	// v2 Labels interface -----------------------------------------------
	template<class TileType>
	void RenderNodeLabels(QuadTreeNode<TileType> *node, VulkanPad *skp, oapi::Font **labelfont, int *fontidx);
	...
	inline oapi::VulkanClient* Client() const { return gc; }
	...
	inline double ElevRes() const { return elevRes; }
...
public:
	static VulkanTexture *hOcean;
	static VulkanTexture *hCloudMicro;
	static VulkanTexture *hCloudMicroNorm;
};
