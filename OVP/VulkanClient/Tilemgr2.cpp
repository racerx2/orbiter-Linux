...
VulkanTexture *LoadDDSFile(const char *path, bool bFullMipChain)
{
	...
	if (bytes > 0) {
		...
		if (fread(data, 1, bytes, f) == (size_t)bytes)
			pTex = NatCreateTextureFromDDSInMemory(data, (size_t)bytes, bFullMipChain);
		delete []data;
	}
	fclose(f);
	return pTex;
...
: mgr(_mgr), lvl(_lvl), ilat(_ilat), ilng(_ilng),
  ...
  owntex (true), ownoverlay(false),
  texrange(fullrange), microrange(fullrange), overlayrange(fullrange),
  mesh(NULL), cnt(Centre()),
  edgeok(false),
  state(Invalid),
...
bool Tile::LoadTextureFile(const char *fullpath, VulkanTexture **pPre)
{
	*pPre = NULL;
...
void Tile::Render()
{
	if (mgr->gc->IsControlPanelOpen())
		if (VulkanStats.TilesRendered.count(mgr->GetScene()->GetRenderPass()))
			VulkanStats.TilesRendered[mgr->GetScene()->GetRenderPass()]++;
}
...
bool Tile::Pick(const FMATRIX4 *pW, const FVECTOR3 *vDir, TILEPICK &result)
{
	...
	FVECTOR3 bs = oapi::TransformCoord(mesh->bsCnt, *pW);
	...
	dir = oapi::TransformNormal(*vDir, mWI);
	...
	for (DWORD i = 0; i<mesh->nf; i++)
	{
		...
		if (dot(cp, dir)<0) {
			if (IntersectTri(_c, _b, _a, pos, dir, &u, &v, &dst)) {
				if (dst > 0.1f) {
					if (dst < result.d) {
						idx = i;
						result.d = dst;
	...
	if (idx >= 0) {
		...
		cp = cross(_c - _b, _a - _b);
		cp = oapi::TransformNormal(cp, *pW);
		result._n = unit(cp);

		FVECTOR3 p = (_b * u) + (_a * v) + (_c * (1.0f - u - v));
		result._p = oapi::TransformCoord(p, *pW);
...
//	const float TEX2_MULTIPLIER = 1.0f; // was: 4.0f was: 16.0f
	...
	for (i = n = 0; i <= grdlat; i++) {
		...
		for (j = 0; j <= grdlng; j++) {
			...
			vtx[n].tu0 = FVAL((c1*j)/grdlng+c2); // overlap to avoid seams
			vtx[n].tv0 = FVAL(grdlat-i)/FVAL(grdlat);
...
VBMESH *Tile::CreateMesh_hemisphere (int grd, float *elev, double globelev)
{
    ...
    for (y = 1; y < grd; y++) {
        ...
        for (x = 0; x < x2; x++) {
			...
			pos = nml*eradius;
			vtx->x = FVAL(pos.x);  vtx->nx = FVAL(nml.x);
			vtx->y = FVAL(pos.y);  vtx->ny = FVAL(nml.y);
			vtx->z = FVAL(pos.z);  vtx->nz = FVAL(nml.z);
...
TileLoader::TileLoader (const oapi::VulkanClient *gclient)
	: gc(gclient)
...
VulkanDevice *TileManager2Base::pDev = NULL;
...
VulkanTexture *TileManager2Base::hOcean = NULL;
VulkanTexture *TileManager2Base::hCloudMicro = NULL;
VulkanTexture *TileManager2Base::hCloudMicroNorm = NULL;
...
void TileManager2Base::GlobalInit (class oapi::VulkanClient *gclient)
{
	gc = gclient;
	pDev = gc->GetDevice();
