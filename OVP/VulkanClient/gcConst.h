...
public:
	...
	// A VIRTUAL DESTRUCTOR, WHICH THE WINDOWS CLASS DOES NOT DECLARE.
	//
	// gcConst is polymorphic -- every method below it is virtual -- and
	// VulkanClient.cpp's ExitModule does `delete g_pConst`, where g_pConst is
	// a gcConst*. With a non-virtual destructor that is undefined behaviour
	// the moment anything derives from it, and gcCore/gcCore2 do exactly
	// that. MSVC's warning for this (C4265) is off by default, which is why
	// it was never noticed; GCC reports it under -Wall as
	// -Wdelete-non-virtual-dtor. Adding the declaration is the fix, and it
	// costs one vtable slot on a class that already has a vtable.
	virtual ~gcConst() {}
	...
	*/
	...
	{
		// PickGround holds FVECTOR3s, which have user-declared constructors,
		// so GCC reports the raw clear (-Wclass-memaccess); MSVC's C4995 has
		// no equivalent here and it stays silent. Zeroing is what this stub
		// means -- it is the "no data" answer of an obsolete entry point --
		// and the cast says so without changing a byte.
