...
public:
	 ...
	 * VulkanConfig::DisableVisualHelperReadout value.
	 * \param Config A reference to the configuration class, to get the \ref
	 *   VulkanConfig::DisableVisualHelperReadout value.
	 */
	static void GlobalInit(const VulkanConfig &Config);
	...
	// promising a protection it never provided. MSVC is silent about it,
	// which is why it survived.
	static bool RunsOrbiter2010 () { return isOrbiter2010; }
	 ...
	 */
	static bool RunsOrbiterSound40() { return orbiterSound40; }

	/**
	 * \brief Returns whether Orbiter runs under WINE
	 *
	 * \return Whether Orbiter runs under WINE. Always false in this client:
	 *   it is a native ELF module loaded by a native Linux Orbiter, so there
	 *   is no WINE layer it could be running on top of. Kept because callers
	 *   read it (VulkanFrame.cpp logs it) and because the answer is a real
	 *   answer, not a stub.
	 */
	static bool RunsUnderWINE() { return runsUnderWINE; }
	...
	*/
	static bool RunsSpacecraftDll() { return runsSpacecraftDll; }
...
private:
	...
	// LogD3D9Modules() is gone -- see OapiExtension.cpp.
