...
// ==============================================================
//
// CONVERTED FROM OVP/D3D9Client/TileMgr.cpp, read end to end (1294 lines).
//
// The LEGACY (v1) tile manager. Its file readers, its quadtree walk, its
// LOD arithmetic and its whole tile-cache-with-loader-thread convert
// unchanged -- none of that touches a graphics API on either platform, and
// the threading is Win32 rather than Direct3D, which the shim already
// supplies over pthreads.
//
// FIVE THINGS CHANGE, and four of them are mechanical:
//
//   1. ReleaseTex. Was pTex->Release() -- a COM reference count. A
//      VulkanTexture knows the VkDevice that made it (VulkanTexture::vkOwner)
//      and its destructor destroys the image, the view and the memory, so
//      this is `delete`. That is also exactly what
//      VulkanDevice::DestroyTexture does, and this function has no device to
//      call it on.
//
//   2. The D3DMAT_ helpers -> VMAT_. These are the CLIENT'S OWN functions,
//      renamed with the rest; only their names referred to Direct3D.
//      D3DXMatrixMultiply -> VMAT_MatrixMultiply, which is aliasing-safe --
//      Render() and SetWorldMatrix both pass a matrix as input and output.
//
//   3. D3DXVec3TransformCoord / D3DXVec3Length -> DrawAPI.h's TransformCoord
//      and a written-out length. D3DX is a Direct3D UTILITY library with no
//      Vulkan counterpart.
//
//   4. dev->GetViewport() in GlobalInit -- a VkDevice answers no questions
//      about itself, and a viewport is not device state in Vulkan at all. The
//      render target's size comes from CVulkanFramework, which is what those
//      four numbers were always for. Same change as CSphereMgr::GlobalInit.
//
//   5. ReadDDSSurface. THIS IS THE ONE THAT IS NOT MECHANICAL, and it is
//      worth reading in full at the function.
//
// `#include <ddraw.h>` stays as written: the shim supplies DDSURFACEDESC2,
// DDPIXELFORMAT and DDSD_LINEARSIZE (Src/Orbiter/Linux/ddraw.h), and this
// file uses them to parse a FILE FORMAT, not to call DirectDraw.
// ==============================================================
...
// ADDED. ProcessTile and IsTileInView call gc->GetScene()->IsVisibleInCamera,
// which needs the complete Scene. On Windows D3D9Client.h pulled Scene.h in
// for everyone; VulkanClient.h does not, so the file that uses it says so.
#include "Scene.h"
...
// Local prototypes
//
// Was pTex->Release(): the COM reference count, which for a texture nobody
// else held meant "destroy it". A VulkanTexture is not reference counted and
// carries the VkDevice that made it, so its destructor is the destruction.
// VulkanDevice::DestroyTexture is `delete pTex` for the same reason, and this
// function has no device to reach it through.
...
TileManager::TileManager (VulkanClient *gclient, const vPlanet *vplanet) : VulkanEffect()
{
	vp = vplanet;
	obj = vp->Object();
	char name[256];
	...
	specbuf  = NULL;
	cAmbient = 0;
	// See TileMgr.h: the pass's own cull unless a caller overrides it.
	cullMode = VulkanEffectFile::PassOverride::CULL_PASS;
	bNoTextures = false;
...
bool TileManager::AddSubtileData (TILEDESC &td, TILEFILESPEC *tfs, DWORD idx, DWORD sub, DWORD lvl)
{
	...
	if (t.flags || bSubtiles) {
		if ((int)lvl <= maxlvl) {
			...
			// The cast is the Windows one with the pointer type changed. See
			// the note jarmonik left on AddSubtileTextures: this field holds
			// an INDEX until the textures are loaded and a POINTER after,
			// which is why an integer is being cast to a pointer here.
			td.subtile[sub]->tex = (VulkanTexture *)t.sidx;
			if (bGlobalSpecular || bGlobalLights) {
				if (t.midx != NOTILE) {
					td.subtile[sub]->ltex = (VulkanTexture *)t.midx;
				}
			} else {
...
void TileManager::LoadTextures (char *modstr)
{
	...
	// condition, which GCC flags (-Wparentheses) because it is usually a typo
	// for ==. It is deliberate here, and the extra parentheses say so.
	if ((ntex = LoadPlanetTextures(fname, texbuf, 0, ntex)) != 0) {
...
void TileManager::PreloadTileTextures (TILEDESC *tile8, DWORD ntex, DWORD nmask)
{
	...
	char fname[256];
	DWORD i, j, nt = 0, nm = 0;
	VulkanTexture **texbuf = NULL, **maskbuf = NULL;
	...
	if (nmask) { // load mask/light textures
		maskbuf = new VulkanTexture *[nmask];
		strcpy_s (fname, 256, objname);
...
void TileManager::AddSubtileTextures (TILEDESC *td, VulkanTexture **tbuf, DWORD nt, VulkanTexture **mbuf, DWORD nm)
{
	DWORD i;
...
void TileManager::LoadSpecularMasks ()
{
	if (nmask) {
		...
		specbuf = new VulkanTexture *[nmask];
		if ((n = LoadPlanetTextures(fname, specbuf, 0, nmask)) != 0) {	// see LoadTextures
			if (n < nmask) {
...
void TileManager::SetAmbientColor(DWORD c)		// was D3DCOLOR
{
	cAmbient = c;
}
...
void TileManager::Render(VulkanDevice *dev, FMATRIX4 &wmat, double scale, int level, double viewap, bool bfog)
{
	VECTOR3 gpos;
	FMATRIX4 imat;
	...
	RenderParam.dev = dev;
	// D3DMAT_ was the client's own helper set; it became VMAT_.
	VMAT_Copy (&RenderParam.wmat, &wmat);
	VMAT_Copy (&RenderParam.wmat_tmp, &wmat);
	VMAT_MatrixInvert (&imat, &wmat);
	// imat._41.._43 are imat.m41..m43 -- the same three elements under
	// FMATRIX4's naming.
	...
	if (level <= 4) {
		...
		for (hemisp = idx = 0; hemisp < 2; hemisp++) {
			if (hemisp) { // flip world transformation to southern hemisphere
				// The destination is also an input; VMAT_MatrixMultiply builds
				// into a local and assigns at the end, so that is safe here as
				// it was for D3DXMatrixMultiply.
				VMAT_MatrixMultiply(&RenderParam.wmat, &Rsouth, &RenderParam.wmat);
	...
	const TEXCRDRANGE &bkp_range, VulkanTexture *bkp_tex, VulkanTexture *bkp_ltex, DWORD bkp_flag)
{
	...
	float bsScale = VMAT_BSScaleFactor(&mWorld);
	...
	if (bStepDown) {
		...
		// D3DXVec3TransformCoord + D3DXVec3Length. The first is DrawAPI.h's
		// TransformCoord (same row-vector convention); the second is three
		// multiplications and a square root, and D3DX has no counterpart to
		// call here.
		FVECTOR3 vBS = TransformCoord(mesh->bsCnt, mWorld);
		float dist = sqrt(vBS.x*vBS.x + vBS.y*vBS.y + vBS.z*vBS.z);
...
void TileManager::SetWorldMatrix (int ilng, int nlng, int ilat, int nlat)
{
	...
	double lng = PI*2.0 * (double)ilng/(double)nlng + PI; // add pi so texture wraps at +-180 deg
	VMAT_RotY (&rtile, lng);
	...
	if (nlat > 8) {
		...
		VMAT_MatrixMultiply(&mWorld, &rtile, &RenderParam.wmat_tmp);
	} else {
		VMAT_MatrixMultiply(&mWorld, &rtile, &RenderParam.wmat);
	}

	(void)wtrans;	// declared and unused on Windows too
}

// ==============================================================

bool TileManager::SpecularColour (COLOUR4 *col)		// was D3DCOLORVALUE
{
	if (!atmc) {
...
void TileManager::GlobalInit (VulkanClient *gclient)
{
	...
	//
	// Was D3DVIEWPORT9 vp; dev->GetViewport(&vp). A VkDevice answers no
	// questions about itself, and a VIEWPORT IS NOT DEVICE STATE in Vulkan --
	// it is a value written into a pipeline or a command buffer. The render
	// target's size is what these four numbers are for, and it lives on
	// CVulkanFramework. The X/Y origin was always 0 for the full-target
	// viewport this client sets. Same change as CSphereManager::GlobalInit.
...
//
// NOT CONVERTED, and deliberately: the whole of it is Win32, not Direct3D.
// CreateMutex, CreateThread, CreateEvent, SetEvent, ResetEvent, CloseHandle,
// WaitForSingleObject, ReleaseMutex and `DWORD WINAPI` are all supplied by
// the shim over pthreads (Src/Orbiter/Linux/windows.h:1220-1226, with WINAPI
// defined empty). Rewriting this on std::thread would be reinventing working
// code. Only the texture TYPE and ReadDDSSurface change.

// The list is reordered to the declaration order -- gc, bLoadMip, nbuf,
// nused, last. bLoadMip is declared immediately after gc and was initialised
// last; members run in declaration order regardless. Every value is a
// constant, so nothing observable changes. Same finding as Objmgr's,
// ScatterParams's, TreeFileHeader's and VBMESH's.
TileBuffer::TileBuffer (const oapi::VulkanClient *gclient)
	: gc(gclient)
	, bLoadMip(true)
	, nbuf(0)
	, nused(0)
...
DWORD WINAPI TileBuffer::LoadTile_ThreadProc (void *data)
{
	...
	while (bFirstRun || WAIT_OBJECT_0 != WaitForSingleObject(hStopThread, idle))
	{
		...
		if ((load = (nqueue > 0)) != false) {		// see LoadTextures
		...
		if (load) {
			...
			if (tidx == NOTILE)
				...
				if (hr != S_OK) {
					tex = NULL;
					LogErr("Failed to load a tile using ReadDDSSurface() offset=%u, name=%s, ErrorCode = %d",(unsigned)ofs,fname,(int)hr);
				}
			}
...
// =======================================================================
// THE ONE FUNCTION IN THIS FILE THAT IS NOT A RENAME.
//
// It reads a DDS surface out of the middle of a .tex archive: seek to an
// offset, check the magic, read the header, and hand the compressed bytes to
// the GPU. THE PARSING HALF IS UNCHANGED -- DDSURFACEDESC2 and MAKEFOURCC
// describe a FILE FORMAT, not a DirectDraw call, and the shim supplies both.
//
// The upload half is where D3D9 and Vulkan genuinely differ, and the
// difference is the whole reason the Windows function has two branches:
//
//   bManaged=true  -> D3DPOOL_MANAGED, LockRect, fread straight into the
//                     mapped pointer, UnlockRect. The RUNTIME then uploaded
//                     it to video memory whenever it felt like it, and would
//                     re-upload after a device loss. That pool does not
//                     exist here and cannot: nothing in Vulkan keeps a
//                     shadow copy for you.
//
//   bManaged=false -> D3DPOOL_DEFAULT for the real texture, a second
//                     D3DPOOL_SYSTEMMEM texture as scratch, LockRect the
//                     scratch, fread into it, UpdateTexture to copy across.
//                     That IS the Vulkan model, spelled in D3D9 terms: a
//                     host-visible staging copy and an explicit transfer.
//
// So BOTH branches collapse into one path -- UploadTexture, which is exactly
// "staging buffer plus vkCmdCopyBufferToImage plus the layout transitions"
// (see VulkanDevice::UploadTexture). bManaged is kept as a parameter because
// it is part of the declared interface and every caller passes false; it no
// longer selects anything, and the comment says so rather than the code
// pretending otherwise.
//
// The return values are unchanged, including the negative ones: they are this
// function's OWN status codes, not a device's, which is why this still
// returns HRESULT after HR() was repointed at VkResult.

HRESULT TileBuffer::ReadDDSSurface (VulkanDevice *pDev, const char *fname, LONG_PTR ofs, VulkanTexture **pTex, bool bManaged)
{
	_TRACE;
	char cpath[256];
	...
	if (!fread(&dwMagic, sizeof(DWORD), 1, f))	{ fclose(f); return -2; }

	if (dwMagic != MAKEFOURCC('D','D','S',' ')) { fclose(f); return -4; }

	// Read the surface description
	fread(&ddsd, sizeof(DDSURFACEDESC2), 1, f);

	// D3DFORMAT -> VkFormat. The three BC block formats the file can hold,
	// under their Vulkan names; the mapping is the one fixed in
	// VulkanSurface.cpp.
	VkFormat Format;
	...
	*pTex = NULL;
	...
	// DDSD_LINEARSIZE is what says "dwLinearSize holds the byte count of the
	// compressed level". Without it there is no length to read, which is what
	// the Windows function's fall-through to -7 meant.
	if ((ddsd.dwFlags & DDSD_LINEARSIZE) == 0) {
		fclose(f);
		return -7;
	}

	VulkanTexture *pNew = pDev->CreateTexture(ddsd.dwWidth, ddsd.dwHeight, 1, Format,
	...
	// The Windows code freads STRAIGHT INTO the mapped texture. Vulkan
	// device-local memory cannot be mapped at all, so the bytes land in a
	// heap block first and UploadTexture stages them across. That is the same
	// two steps the D3DPOOL_SYSTEMMEM branch took, with the scratch texture
	// replaced by a scratch buffer inside UploadTexture.
	BYTE *pData = new BYTE[ddsd.dwLinearSize];
	if (fread(pData, 1, ddsd.dwLinearSize, f) != ddsd.dwLinearSize) {
		delete []pData;
		pDev->DestroyTexture(pNew);
		fclose(f);
		return -7;
	}

	bool ok = pDev->UploadTexture(pNew, 0, 0, pData, ddsd.dwLinearSize);
	delete []pData;
	fclose(f);
...
struct TileBuffer::QUEUEDESC TileBuffer::loadqueue[MAXQUEUE] = {};
