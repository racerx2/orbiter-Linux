...
//	double size = oapiGetSize(hPlanet);
	(void)hPlanet;	// fetched and, with the line above commented out, unread
...
void RunwayLights::SetPAPIColors(BeaconArray *pPAPI, FMATRIX4 *world, int i)
{
	...
	if (pVrt) {
		...
		float slope = float(-asin(dot(vFront, vUp))*DEG);
...
void RunwayLights::Render(VulkanDevice *dev, FMATRIX4 *world, bool night)
{
	_TRACE;
	...
	FVECTOR3 dirGlo = oapi::TransformNormal(FVEC(dir), *world);
	
	if (dot(FVEC(camDir), dirGlo) > 0)
	{
	...
	while(fgets2(cbuf, 256, file)>=0)
	{
		if(!strncmp(cbuf, "RUNWAYLIGHTS", 12))
		{
			...
			for(;;)
			{
				...
				{
					VECTOR3 vec; DWORD e;
					// "%lu" on Windows -- see the PAPI note above.
...
void TaxiLights::Render(VulkanDevice *dev, FMATRIX4 *world, bool night)
{
