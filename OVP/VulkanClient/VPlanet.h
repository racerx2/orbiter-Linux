...
{
public:
	PlanetShader(VulkanDevice *pDev, const char* file, const char* vs, const char* ps, const char* name, const char* options);
	~PlanetShader();
...
struct ShaderParams
{
	FMATRIX4P mWorld;			// World Matrix
	FMATRIX4P mLVP;				// Light-View-Projection
	FVECTOR4P vSHD;				// Shadow Map Parameters
	FVECTOR4P vMSc[3];			// Micro Texture offset-scale
	FVECTOR4P vTexOff;			// Texture offset-scale
	FVECTOR4P vCloudOff;		// Cloud texture offset-scale
	FVECTOR4P vMicroOff;		// Micro texture offset-scale
	FVECTOR4P vOverlayOff;      // Overlay texture offset-scale
	FVECTOR4P vOverlayCtrl[4];
...
struct ConstParams
{
	FMATRIX4P mVP;				// View Projection Matrix
	...
	FVECTOR4P HG;				// Henyey-Greenstein Phase function params
...
static_assert(offsetof(ShaderParams, mLVP) == 64, "ShaderParams::mLVP moved");
static_assert(offsetof(ShaderParams, vEclipse) == 320, "ShaderParams::vEclipse moved");
static_assert(sizeof(ShaderParams) == 348, "ShaderParams must stay tightly packed for shader upload");
static_assert(offsetof(ConstParams, CamPos) == 64, "ConstParams::CamPos moved");
static_assert(offsetof(ConstParams, HG) == 208, "ConstParams::HG moved");
static_assert(offsetof(ConstParams, PlanetRad) == 272, "ConstParams::PlanetRad moved");
static_assert(sizeof(ConstParams) == 432, "ConstParams must stay tightly packed for shader upload");
...
public:
	...
	static void		GlobalInitAtmosphere(oapi::VulkanClient* gc);
	...
	void			SetMicroTexture(VulkanTexture *pSrc, int slot);
	...
	sOverlay *		AddOverlaySurface(VECTOR4 lnglat, gcCore::OlayType type, VulkanTexture *pSrf = NULL, sOverlay *pOld = NULL, const FVECTOR4* pB = NULL);
	...
	VulkanTexture * GetScatterTable(int i);
	...
	VulkanSun		GetObjectAtmoParams(VECTOR3 relpos);
	...
	VulkanTexture * GetEclipse() { return ptEclipse; }

	// v2 Labels interface ----------------------------------------------------
	void            ActivateLabels(bool activate);
	void            RenderLabels(VulkanDevice *dev, VulkanPad *skp, oapi::Font **labelfont, int *fontidx);
...
protected:
	void RenderSphere (VulkanDevice *dev);
	void RenderCloudLayer (VulkanDevice *dev, DWORD cullmode);
	void RenderBaseSurfaces (VulkanDevice *dev);
	void RenderBaseStructures (VulkanDevice *dev);
	void RenderBaseShadows (VulkanDevice *dev, float depth);
	void RenderCloudShadows (VulkanDevice *dev);
	bool ModLighting (DWORD &ambient);

	bool ParseMicroTextures();            ///< Read micro-texture config for this planet
	static void LoadMicroTextures(VulkanDevice *pDev);

private:

	VulkanTexture *pSunColor, *pRaySkyView, *pMieSkyView, *pLandViewRay, *pLandViewMie, *pAmbientSky, *pLandViewAtn;
	...
	static VulkanTexture *ptEclipse;
	static int Qc, Wc, Nc;
	...
	VulkanMesh *mesh;         // mesh for non-spherical body
