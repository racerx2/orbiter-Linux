...
inline gcCore2* gcGetCoreInterface()
{
	...
	// The reference asks for "D3D9Client.dll" -- the graphics client's own
	// module file. Here the graphics client is VulkanClient, and the name is
	// given without an extension: the Linux GetModuleHandle matches on the
	// lower-cased stem through the loader index orb_LoadLibrary maintains, so
	// this finds Modules/Plugin/VulkanClient.so however it was loaded. Leaving
	// the Windows name here is not harmless -- it returns NULL, every add-on
	// that draws through gcCore silently loses its interface, and the only
	// trace is one line in the log.
	...
	} else oapiWriteLogV("gcGetCoreInterface() FAILED. VulkanClient Not Found");
	return NULL;
}
