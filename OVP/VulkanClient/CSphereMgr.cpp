...
const VulkanConfig *CSphereManager::cfg = NULL;
...
CSphereManager::CSphereManager(VulkanClient *gc, const Scene *scene) : gc(gc), texname(), RenderParam()
{
	scn = scene;
...
void CSphereManager::GlobalInit(oapi::VulkanClient *gclient)
{
	...
	diagscale = (double)vpW/(double)vpH;
...
void CSphereManager::LoadTextures ()
{
	...
	if (m_bStarImg) {
		...
		if ((ntex = LoadPlanetTextures(fname, m_starbuf.data(), 0, ntex)) != 0) {
			while (ntex < patchidx[lvl])
				--lvl;
			while (ntex > patchidx[lvl])
				ReleaseTex(m_starbuf[--ntex]);
			if (ntex < (int)m_starbuf.size())
				m_starbuf.resize(ntex);
		}
		else {
			m_starbuf.clear();
	...
	if (m_bBkgImg && m_bStarImg && texlvl != starlvl) {
		if (texlvl < starlvl) {
			for (size_t i = m_texbuf.size(); i < m_starbuf.size(); i++)
				ReleaseTex(m_starbuf[i]);
			m_starbuf.resize(m_texbuf.size());
			starlvl = texlvl;
		}
		else {
			for (size_t i = m_starbuf.size(); i < m_texbuf.size(); i++)
...
void CSphereManager::Render (VulkanDevice *dev, int level, double bglvl)
{
...
void CSphereManager::SetWorldMatrix (int ilng, int nlng, int ilat, int nlat)
{
	...
	double lng = PI2 * (double)ilng/(double)nlng + PI; // add pi so texture wraps at +-180 deg
	VMAT_RotY (&rtile, lng);
