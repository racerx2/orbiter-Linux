...
struct TreeNode {
	...
	TreeNode () : pos(0), size(0) {
		// ARRAYSIZE yields a size_t, and comparing it against an int is a
		// -Wsign-compare (MSVC's C4018, warned about there too and left in).
		// The cast is the whole change; four is four either way.
		for (int i = 0; i < (int)ARRAYSIZE(child); ++i) child[i] = (DWORD)-1;
	}
};
