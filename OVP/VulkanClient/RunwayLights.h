...
// DrawAPI.h is where FMATRIX4 lives. On Windows <d3dx9.h> supplied
// LPD3DXMATRIX and no SDK header was needed; here the matrix type is the
// SDK's own, so the header that declares it is included by name.
#include "DrawAPI.h"
...
public:
	...
	void Init();
	void Render(VulkanDevice *dev, oapi::FMATRIX4 *world, bool night);
...
protected:
	...
	void			   SetPAPIColors(BeaconArray *pPAPI, oapi::FMATRIX4 *world, int idx);
