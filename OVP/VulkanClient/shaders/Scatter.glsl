...
// ============================================================================
// Scatter.glsl -- converted from OVP/D3D9Client/shaders/Scatter.hlsl
...
// file -- vPlanetAtmo.cpp builds one from it with "SunColor" and then
// CompileShader()s five more entry points -- so its pixel block sits at
// binding 1, its samplers from binding 2, and its vertex shader is IPI.glsl's
// VSMain, which is why every entry point takes `float u : TEXCOORD0, float v
// : TEXCOORD1`.
//
// The one option the client passes is `_PERFORMANCE`, which selects the
// smaller lookup-table dimensions at the top. It arrives through the
// ImageProcessing constructor's third argument.
//
// FOUR THINGS THE LANGUAGE FORCED, each marked again where it happens:
//
//  1. GLSL HAS NO DEFAULT ARGUMENTS. `AmbientApprox(..., uniform const bool
//     bR = true)` becomes two functions plus two one-argument wrappers that
//     pass `true`. Every call site is unchanged.
//
//  2. `uniform` ON A FUNCTION PARAMETER is HLSL for "this argument is a
//     compile-time constant". GLSL has no such qualifier and needs none:
//     every call passes a literal or a struct member, and glslang folds the
//     ones that are constant exactly as fxc did.
//
//  3. `static const float n[] = {...}` NEEDS ITS SIZE AND A CONSTRUCTOR in
//     GLSL: `const float n[NSEG] = float[NSEG](...)`. Same values, same
//     order.
//
//  4. `(RayData)0` HAS NO COUNTERPART. Both structs so initialised have every
//     member assigned before it is read, so the zero-init is dropped rather
//     than written out member by member -- see the notes at each one.
// ============================================================================
...
// Per-Frame Params
//
// The mirror of vPlanet.h's ConstParams, whose size (432) and three interior
// offsets that header asserts. layout(scalar) is what makes the eleven float3
// members here land where the packed C++ struct puts them.
//
struct AtmoParams
{
...
// The mirror of vPlanet.h's sFlow. A bool in a GLSL uniform block is four
// bytes, as BOOL is in C++ and HLSL -- see CelSphere.glsl's header.
struct sFlow {
...
layout(set = 0, binding = 1, scalar) uniform ScatterPSBlock
{
	AtmoParams Const;
	sFlow Flo;
};
...
// `static const float n[]` / `w[]` -- see note 3 in the header.
#if NSEG == 5
...
//
// `const float rc, const float prc` -- see note 2 in the header; the
// qualifier says nothing GLSL needs.
...
//
// The misspelling is the reference's and is kept: it is the name every call
// site uses.
...
//
// `uniform const bool bR = true` -- the default argument becomes a wrapper;
...
//
// `in uniform const bool bPreProcessData` -- see note 2 in the header.
RayData ComputeRayStats(in vec3 vRay, bool bPreProcessData)
{
	// `RayData dat = (RayData)0;` has no counterpart and needs none: all six
	// members are assigned below before anything reads them. See note 4.
	RayData dat;
	...
	// This local shadows the file-scope `n[]` table. That is the reference's
	// own naming and GLSL allows it exactly as HLSL does -- a declaration
	// takes effect from its own declarator onward -- so nothing here reads
	// the table, and IntegrateSegmentNS below still does.
...
vec4 IntegrateSegmentNS(vec3 vOrig, vec3 vRay, float len, float iH)
{
	...
	for (int i = 0; i < NSEG; i++)
	{
		...
		// `float3 n = normalize(pos);` in the reference, declared AFTER the
		// line above that reads the file-scope `n[]` table. That is legal in
		// both languages, but it reads as a mistake; the local is `nn` here
		// and the table keeps its name.
		vec3 nn = normalize(pos);
		float rad = dot(nn, pos);
		float alt = rad - Const.PlanetRad;
		vec3 x = GetSunColor(dot(nn, Const.toSun), alt);
...
//
#if defined(_EP_SunColor)
void SunColor()
{
	float maxalt = max(Const.MaxAlt, Const.CloudAlt);
	float alt = mix(Const.MinAlt, maxalt, psY*psY);
	float ang = psX * ANGRNG + MINANGLE;
	float rad = alt + Const.PlanetRad;
	float dist = RayLength(-ang, rad);
	vec2 rm = Gauss7(-ang, rad, dist, Const.iH) * Const.rmO;
	vec3 clr = Const.RayWave * rm.r + Const.MieWave * rm.g;

	oColor = vec4(exp(-clr), 1.0f);
}
#endif // _EP_SunColor
...
#if defined(_EP_SkyView)
void SkyView()
{
	// Viewing ray
	vec3 vRay = uvToDir(vec2(psX,psY));
	...
	if (Flo.bRay)
	{
		...
		oColor = vec4(ret.rgb + cMlt.rgb, alpha);
		return;
	}

	oColor = vec4(ret.rgb, 1.0f);
}
#endif // _EP_SkyView
...
//
#if defined(_EP_RingView)
void RingView()
{
	float v = psY;

	if (v >= LastLine) { oColor = vec4(0, 0, 0, 0); return; }
	...
	if (Flo.bRay)
	{
		...
		cMlt.rgb *= exp(-(Const.RayWave * vOut.r + Const.MieWave * vOut.g));
		float alpha = ilerp(10e3, 150e3, vDepth.r);
		alpha = alpha > 0.0f ? sqrt(alpha) : 0.0f;
		oColor = vec4(ret.rgb + cMlt.rgb, alpha);
		return;
	}
...
//
#if defined(_EP_AmbientSky)
void AmbientSky()
{
	// Viewing ray
	vec2 uv = vec2(psX, psY);
...
void LandView()
{
	...
	vec3 vNrm = uvToNrm(vec2(fract(u), psY));
	vec3 vVrt = vNrm * r;
	...
	float s0 = sp.ae > 0.0f ? sp.ae : 0.0f;
	float e0 = sp.se > 0.0f ? min(cpd, sp.se) : cpd;
